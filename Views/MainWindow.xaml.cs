...
namespace WinIsland
{
    public partial class MainWindow : Window
    {
        ...
        private void CleanupResources()
        {
            ...
            try { DisposeAiTaskMonitor(); } catch { }
            try
            {
        ...
        {
            if (e.PropertyName == nameof(ViewModels.MediaViewModel.Title) ||
                e.PropertyName == nameof(ViewModels.MediaViewModel.Artist))
            {
                Dispatcher.Invoke(() =>
                {
        ...
        private void CenterWindowAtTop()
        {
            ...
            {
                var settings = AppSettings.Load();
                if (settings.HasCustomIslandPosition)
                {
                    this.Left = settings.IslandLeft;
                    this.Top = settings.IslandTop;
                    return;
                }
        ...
        private void HideAllPanels()
        {
            ...
            AiTaskPanel.Visibility = Visibility.Collapsed;
        ...
        private bool _isDirectIslandDrag;
        ...
        private void Window_MouseDown(object? sender, MouseButtonEventArgs e)
        {
            ...
            {
                BeginPotentialIslandDrag(e, false);
            }
        }

        private void Window_MouseMove(object? sender, System.Windows.Input.MouseEventArgs e)
        {
            if (_isPotentialDrag && e.LeftButton == MouseButtonState.Pressed && (_isDirectIslandDrag || Keyboard.Modifiers == ModifierKeys.Control))
            {
                    ...
                    Math.Abs(currentPoint.Y - _dragStartPoint.Y) > 5)
                {
                    ...
                    _isDirectIslandDrag = false;
                    DynamicIsland.ReleaseMouseCapture();
                    DragIslandWindow();
                }
            }
            else if (_isPotentialDrag && e.LeftButton != MouseButtonState.Pressed)
            {
                _isPotentialDrag = false;
                _isDirectIslandDrag = false;
                DynamicIsland.ReleaseMouseCapture();
        ...
        private void DynamicIsland_MouseDown(object? sender, MouseButtonEventArgs e)
        {
            ...
            if (!IsInteractiveDragSource(e.OriginalSource))
            {
                BeginPotentialIslandDrag(e, true);
                e.Handled = true;
            }
        }

        private void BeginPotentialIslandDrag(MouseButtonEventArgs e, bool directDrag)
        {
            _dragStartPoint = e.GetPosition(this);
            _isPotentialDrag = true;
            _isDirectIslandDrag = directDrag;
            DynamicIsland.CaptureMouse();
        }

        private void DragIslandWindow()
        {
            try
            {
                DragMove();
        ...
        private void SaveIslandPosition()
        {
            ...
            {
                var settings = AppSettings.Load();
                settings.HasCustomIslandPosition = true;
                settings.IslandLeft = this.Left;
                settings.IslandTop = this.Top;
                settings.Save();
            }
            catch { }
        }
